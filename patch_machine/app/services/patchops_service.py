...
from patch_machine.archive.issue_memory import IssueCluster, PatchCandidate, TestRequirement
from patch_machine.archive.patch_runs import PatchRun
from patch_machine.context.ast_indexer import AstIndexer
...
def _test_commands(candidate_files: list[str], test_files: list[str]) -> list[str]:
    commands = [
        "python -m pytest -q",
        "python -m ruff check .",
        "python -m mypy patch_machine tests",
    ]
    if any(
